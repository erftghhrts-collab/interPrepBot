...
import { cn } from "@/lib/utils";
import { vapi } from "@/lib/vapi.sdk";
import { buildRoleplayAssistant } from "@/constants";
...
const Agent = ({ userName, userId }: { userName: string; userId: string }) => {
  ...
  const handleStartRoleplay = async () => {
    const trimmed = roleplayScenario.trim();

    if (!trimmed) {
      setTransformError("Please describe your roleplay scenario first.");
      return;
    }
    ...
    try {
      // Step 1: Send scenario to Groq → get reframed system prompt
      const res = await fetch("/api/groq/transform-prompt", {
        method: "POST",
      ...
      if (!res.ok || !data.systemPrompt) {
        setTransformError(data.error || "Failed to prepare roleplay. Please try again.");
        setIsTransforming(false);
        return;
      }

      // Step 2: Build dynamic VAPI assistant from transformed prompt
      const roleplayAssistant = buildRoleplayAssistant(data.systemPrompt, data.firstMessage);

      setIsTransforming(false);
      setCallStatus(CallStatus.CONNECTING);

      // Step 3: Start the call
      await vapi.start(roleplayAssistant, {
        variableValues: { username: userName, userid: userId },
      });

    } catch (err) {
      console.error("Roleplay error:", err);
      setTransformError("Something went wrong. Please try again.");
      setIsTransforming(false);
    }
  };
  ...
  const isBusy = isTransforming || callStatus === CallStatus.CONNECTING;

  return (
    <div className="flex flex-col gap-4 w-full max-w-2xl mx-auto">

      {/* Scenario input — hidden once call starts */}
      {callStatus === CallStatus.INACTIVE && (
        <div className="flex flex-col gap-2">
          <label
            htmlFor="roleplay-scenario"
            className="text-sm font-semibold text-white/80"
          >
            Describe your roleplay scenario
          </label>
          <textarea
            ...
            placeholder="Example: I am cold calling an HR to pitch our hiring platform. Play the HR — be rude, disinterested, and raise objections."
            rows={5}
            disabled={isBusy}
            className={cn(
              "w-full resize-none rounded-xl px-4 py-3 text-sm text-white placeholder-white/30",
              "border border-white/10 bg-white/5 backdrop-blur-sm",
              "focus:outline-none focus:ring-2 focus:ring-[#00FFFF]/40 focus:border-[#00FFFF]/40",
          ...
          />
          {transformError && (
            <p className="text-xs text-red-400">{transformError}</p>
          )}
          {isTransforming && (
            <p className="flex items-center gap-1.5 text-xs text-[#00FFFF]/80">
              <span className="size-3 animate-spin rounded-full border border-[#00FFFF]/30 border-t-[#00FFFF]" />
              Preparing your roleplay scenario…
            </p>
          )}
        </div>
      )}
      ...
      <div className="relative w-full h-[480px]">
        <div
          className="relative z-20 h-full w-full overflow-hidden rounded-2xl border border-white/10"
          style={{
            background:
              "radial-gradient(1200px 600px at 20% 20%, rgba(0,255,255,0.12), transparent 60%), radial-gradient(800px 500px at 80% 80%, rgba(202,197,254,0.12), transparent 65%), linear-gradient(180deg, rgba(2,4,8,0.98), rgba(2,4,8,0.78))",
          }}
        >
          ...
          <div className="absolute left-0 top-0 z-20">
            <div
              className="flex items-center gap-3 rounded-br-2xl px-3 py-2"
              style={{
                background: "rgba(0,0,0,0.38)",
                backdropFilter: "blur(14px)",
                borderRight: "1px solid rgba(255,255,255,0.12)",
            ...
            >
              ...
              <div className="flex flex-col">
                <p className="text-sm font-semibold text-white">Roleplay Partner</p>
                <p className="text-[11px] text-white/70">
                  {isBusy
                    ? "Preparing…"
                    : callStatus === CallStatus.ACTIVE
        ...
        {/* Call controls */}
        <div
          className="absolute left-1/2 z-[70] -translate-x-1/2"
          style={{ bottom: "22px" }}
        >
          {callStatus === CallStatus.ACTIVE ? (
            <button
              onClick={handleDisconnect}
              className="inline-flex min-w-32 items-center justify-center rounded-full px-8 py-3 text-sm font-bold text-white cursor-pointer"
              style={{
                background:
                  "linear-gradient(180deg, rgba(247,83,83,0.95), rgba(196,65,65,0.95))",
                boxShadow: "0 16px 44px rgba(0,0,0,0.55)",
            ...
            <button
              ...
              style={{
                background:
                  "linear-gradient(180deg, rgba(0,200,255,0.9), rgba(0,150,200,0.9))",
                boxShadow: "0 16px 44px rgba(0,0,0,0.55)",
              }}
            >
              Try again
            ...
            <button
              ...
              className={cn(
                "inline-flex min-w-32 items-center justify-center rounded-full px-8 py-3 text-sm font-bold text-white",
                isBusy ? "opacity-80 cursor-not-allowed" : "cursor-pointer"
              )}
              style={{
                background:
                  "linear-gradient(180deg, rgba(73,222,80,0.95), rgba(66,199,72,0.95))",
            ...
            >
              {isBusy ? (
                <span className="inline-flex items-center gap-2">
                  ...
                  {isTransforming ? "Preparing…" : "Calling…"}
                </span>
              ) : (
                "Start Roleplay"
              )}
